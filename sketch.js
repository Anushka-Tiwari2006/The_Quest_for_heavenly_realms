...
function preload(){
   ...
   clapping = loadSound("clap_guitar.mp3")
...
function setup() {
  ...
  skull2 = createSprite(12790,400,20,80);
  ...
  pc2 = createSprite(4000,610,20,20)
   pc2.addImage(pcImg)
   pc2.scale = 0.7
  ...
  text1 = createSprite(8900,300,10,10)
  text2 = createSprite(9700,300,10,10)
  text1.addImage(t1)
  text2.addImage(t2)
  ...
  boris.setCollider("rectangle",0,0,200,340);
...
function draw() {
  ...
  if(gameState === "title"){
    ...
    fill("lightyellow")
    textSize(30)
    text("Press A to go to Next>>>", 300,400)
    if(keyDown("a")){
      gameState="slideA"
    }  
    ...
    fill("lightyellow")
    textSize(30)
    text("Press B to go to Next>>>", 900,50)
    ...
    fill("lightyellow")
    textSize(30)
    text("Press C to go to Next>>>", 900,50)
    ...
    fill("lightyellow")
    textSize(30)
    text("Press D to go to Next>>>", 900,50)
    if(keyDown("d")){
      gameState="rulesA"
    }  
  }
  ...
  if(pc.isTouching(zombGroup)||pc.isTouching(ground2)){
    gameState = "end";
    }
...
if (keyWentUp("left")|| keyWentUp("right")) {
  ...
  if (pc2.isTouching(m1)||pc2.isTouching(m2)||pc2.isTouching(m3)||pc2.isTouching(m4)||pc2.isTouching(m5)||pc2.isTouching(m6)||pc2.isTouching(m7)||pc2.isTouching(m8)||pc2.isTouching(m9)||pc2.isTouching(m10)||pc2.isTouching(m11)||pc2.isTouching(m12)||pc2.isTouching(m13)||pc2.isTouching(m14)) {
   ...
   image(poly, Polygon.position.x, Polygon.position.y, 200, 200); 
      ...
      fill("lightyellow")
    textSize(30)
    text("Press H to go to Next>>>",  600,10)
      if(keyDown("h")){
        gameState = "overGame2"
      }
    }
      ...
      fill("lightyellow")
    textSize(30)
    text("Press I to go to Next>>>",  600,10)
    }
      ...
      fill("lightyellow")
    textSize(30)
    text("Refresh the page to play again>>>", 600,10)
